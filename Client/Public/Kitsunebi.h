...
namespace Client
{
	/********************
		Descriptor
	********************/
	struct FKitsunebiDesc final : public FMonsterDesc
	{
	};

	class CKitsunebi final : public CMonster
	{
	/********************
		Ctor/Dtor
	********************/
	public:
		explicit CKitsunebi() = default;

		virtual ~CKitsunebi() = default;

	/********************
		Events
	********************/
	public:
		static std::shared_ptr<CKitsunebi> Create(const std::shared_ptr<FKitsunebiDesc>& _spKitsunebiDesc);
	...
	********************/
	public:
		void InitKitsunebi(const shared_ptr<FKitsunebiDesc>& _spKitsunebiDesc);

		void InitMeshComponent();

		void InitColliderComponent();

		void InitBehaviorTreeComponent(const std::shared_ptr<FKitsunebiDesc>& _spKitsunebiDesc);

	private:
		void InitializeWildPalBT(const std::shared_ptr<FKitsunebiDesc> _spKitsunebiDesc, bool _bInit = true);

		void InitializePartnerPalBT(const std::shared_ptr<FKitsunebiDesc> _spKitsunebiDesc, bool _bInit = true);

		void InitializeSlavePalBT(const std::shared_ptr<FKitsunebiDesc> _spKitsunebiDesc, bool _bInit = true);
