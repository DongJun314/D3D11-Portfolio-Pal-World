...
std::shared_ptr<CHedgeHog> Client::CHedgeHog::Create(const std::shared_ptr<FHedgeHogDesc>& _spHedgeHogDesc)
{
	const std::shared_ptr<CHedgeHog> spHedgeHog = make_shared<CHedgeHog>();
	ENSUREF(spHedgeHog->Initialize(_spHedgeHogDesc), L"Failed to initialize hedgehog.");
	return spHedgeHog;
}

HRESULT Client::CHedgeHog::Initialize(const std::shared_ptr<FGameObjectDesc>& _spGameObjectDesc)
{
	if (FAILED(CMonster::Initialize(_spGameObjectDesc))) { return E_FAIL; }

	shared_ptr<FHedgeHogDesc> spHedgeHogDesc = static_pointer_cast<FHedgeHogDesc>(_spGameObjectDesc);
	InitHedgeHog(spHedgeHogDesc);
	InitMeshComponent();
	InitColliderComponent();
	InitBehaviorTreeComponent(spHedgeHogDesc);
	return S_OK;
}

HRESULT Client::CHedgeHog::BeginPlay()
{
...
int32 Client::CHedgeHog::PreTick(float _fDeltaSeconds)
{
...
int32 Client::CHedgeHog::Tick(float _fDeltaSeconds)
{
...
int32 Client::CHedgeHog::PostTick(float _fDeltaSeconds)
{
...
HRESULT Client::CHedgeHog::EndPlay()
{
...
void Client::CHedgeHog::InitHedgeHog(const shared_ptr<FHedgeHogDesc>& _spHedgeHogDesc)
{
	// gameobject desc
	m_wstrGameObjectName = L"HedgeHog";
	m_eMobilityType = EMobilityType::DYNAMIC;

	// monster desc
	m_eMonsterType = EMonsterType::HEDGE_HOG;
	m_eMonsterSizeType = EMonsterSizeType::SMALL;
}

void Client::CHedgeHog::InitMeshComponent()
{
	// component desc
	...
	// mesh component desc
	tMeshComponentDesc.m_wstrModelFilePath = L"../../Resource/Models/Dat/Anim/Hedgehog.dat";
...
void Client::CHedgeHog::InitColliderComponent()
{
...
void Client::CHedgeHog::InitBehaviorTreeComponent(const std::shared_ptr<FHedgeHogDesc>& _spHedgeHogDesc)
{
	// 1. 야생 팰 비헤이비어 트리
	if (m_eMonsterAIType == EMonsterAIType::WILD_PAL)
	{
		InitializeWildPalBT(_spHedgeHogDesc);
	}

	// 2. 파트너 팰 비헤이비어 트리
	else if (m_eMonsterAIType == EMonsterAIType::PARTNER_PAL)
	{
		InitializePartnerPalBT(_spHedgeHogDesc);
	}

	// 3. 거점 팰 비헤이비어 트리
	else if (m_eMonsterAIType == EMonsterAIType::SLAVE_PAL)
	{
		InitializeSlavePalBT(_spHedgeHogDesc);
	}
...
void Client::CHedgeHog::InitializeWildPalBT(const std::shared_ptr<FHedgeHogDesc> _spHedgeHogDesc, bool _bInit)
{
	...
	// behavior tree node
	{
		...
		// wander sequence node
		{
			...
			// move task node
			{
				...
				tTaskMoveDesc.m_vSpawnPosition = _spHedgeHogDesc->m_vInitPosition;
...
void Client::CHedgeHog::InitializePartnerPalBT(const std::shared_ptr<FHedgeHogDesc> _spHedgeHogDesc, bool _bInit)
{
	shared_ptr<CGameObject> spHedgeHog = shared_from_this();
	...
	// behavior tree node
	{
		...
		// partner skill sequence
		{
			...
			// partner skill deco node
			{
				...
				tDecoPartnerSkillDesc.m_spOwnerGameObject = spHedgeHog;
...
void Client::CHedgeHog::InitializeSlavePalBT(const std::shared_ptr<FHedgeHogDesc> _spHedgeHogDesc, bool _bInit)
{
	shared_ptr<CGameObject> spHedgeHog = shared_from_this();
...
HRESULT Client::CHedgeHog::ReInitializePalBT(EMonsterAIType _eMonsterAIType)
{
...
void Client::CHedgeHog::LookAt(const Vector3& _vPosition)
{
...
void Client::CHedgeHog::Turn(const Vector3& _vAxis, float _fAngularSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::MoveForward(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::MoveBackward(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::MoveRight(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::MoveLeft(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::MoveUp(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::MoveDown(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::SetPosition(const Vector3& _vPosition)
{
	if (!m_spBodySphereComponent) { return; }
...
const std::optional<Vector3> Client::CHedgeHog::GetPosition() const
{
...
void Client::CHedgeHog::UpdateBlackboardValues()
{
	CMonster::UpdateBlackboardValues();
}

void Client::CHedgeHog::OnTriggerEnter(CComponent* _pSrcComponent, CComponent* _pDstComponent)
{
...
void Client::CHedgeHog::OnCollisionEnter(CComponent* _pSrcComponent, CComponent* _pDstComponent)
{
	CMonster::OnCollisionEnter(_pSrcComponent, _pDstComponent);
}

void Client::CHedgeHog::SetAllyCollisionLayer()
{
...
shared_ptr<CColliderComponent> Client::CHedgeHog::GetBodyColliderComponent()
{
	return m_spBodySphereComponent;
}

std::shared_ptr<CColliderComponent> Client::CHedgeHog::GetAttackColliderComponent()
{
	return m_spAttackSphereComponent;
}

void Client::CHedgeHog::SetLinearVelocity(const PxVec3& _vLinearVelocity)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CHedgeHog::SetAngularVelocity(const PxVec3& _vLinearVelocity)
{
	if (!m_spBodySphereComponent) { return; }
