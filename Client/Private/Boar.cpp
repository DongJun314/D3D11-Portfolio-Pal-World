...
std::shared_ptr<CBoar> Client::CBoar::Create(const std::shared_ptr<FBoarDesc>& _spBoarDesc)
{
	const std::shared_ptr<CBoar> spPal = make_shared<CBoar>();
	ENSUREF(spPal->Initialize(_spBoarDesc), L"Failed to initialize boar.");
	return spPal;
}

HRESULT Client::CBoar::Initialize(const std::shared_ptr<FGameObjectDesc>& _spGameObjectDesc)
{
	if (FAILED(CMonster::Initialize(_spGameObjectDesc))) { return E_FAIL; }

	shared_ptr<FBoarDesc> spPalDesc = static_pointer_cast<FBoarDesc>(_spGameObjectDesc);
	InitBoar(spPalDesc);
	InitMeshComponent();
	InitColliderComponent();
	InitBehaviorTreeComponent(spPalDesc);
	return S_OK;
}
...
void Client::CBoar::InitBoar(const std::shared_ptr<FBoarDesc>& _spBoarDesc)
{
	// gameobject desc
	m_wstrGameObjectName = L"Boar";
	m_eMobilityType = EMobilityType::DYNAMIC;

	// monster desc
	m_eMonsterType = EMonsterType::BOAR;
	m_eMonsterSizeType = EMonsterSizeType::SMALL;
}

void Client::CBoar::InitMeshComponent()
{
	// component desc
	...
	// mesh component desc
	tMeshComponentDesc.m_wstrModelFilePath = L"../../Resource/Models/Dat/Anim/Boar.dat";
...
void Client::CBoar::InitColliderComponent()
{
	// body sphere component
	{
		...
		tSphereComponentDesc.m_fRadius = 0.65f;
...
void Client::CBoar::InitBehaviorTreeComponent(const std::shared_ptr<FBoarDesc>& _spBoarDesc)
{
	// 1. 야생 팰 비헤이비어 트리
	if (m_eMonsterAIType == EMonsterAIType::WILD_PAL)
	{
		InitializeWildPalBT(_spBoarDesc);
	}

	// 2. 파트너 팰 비헤이비어 트리
	else if (m_eMonsterAIType == EMonsterAIType::PARTNER_PAL)
	{
		InitializePartnerPalBT(_spBoarDesc);
	}

	// 3. 거점 팰 비헤이비어 트리
	else if (m_eMonsterAIType == EMonsterAIType::SLAVE_PAL)
	{
		InitializeSlavePalBT(_spBoarDesc);
	}
...
void Client::CBoar::InitializeWildPalBT(const std::shared_ptr<FBoarDesc> _spBoarDesc, bool _bInit)
{
	shared_ptr<CGameObject> spBoar = shared_from_this();
	...
	if (_bInit)
	{
		...
		tBTComponentDesc.m_wpOwnerGameObject = spBoar;
	...
	// behavior tree node
	{
		...
		tSelectorNodeDesc.m_spOwnerGameObject = spBoar;
		...
		tSequenceNodeDesc.m_spOwnerGameObject = spBoar;
		...
		tDecoBBBoolDesc.m_spOwnerGameObject = spBoar;
		...
		// dead task node
		{
			...
			tTaskDeadDesc.m_spOwnerGameObject = spBoar;
			tTaskDeadDesc.m_wstrBTNodeName = L"DeadTask";
			tTaskDeadDesc.m_eBTNodeType = EBTNodeType::TASK;

			// dead task desc
			tTaskDeadDesc.m_iAnimationID = (int32)EBoar::AS_Boar_SleepStart;
		...
		// captured task node
		{
			...
			// captured task node
			{
				...
				tTaskCapturedDesc.m_spOwnerGameObject = spBoar;
		...
		// jump sequence node
		{
			...
			// jump loop task node
			{
				...
				tTaskJumpLoopDesc.m_spOwnerGameObject = spBoar;
				tTaskJumpLoopDesc.m_wstrBTNodeName = L"JumpLoopTask";
				tTaskJumpLoopDesc.m_eBTNodeType = EBTNodeType::TASK;

				// move task desc
				tTaskJumpLoopDesc.m_iAnimationID = (int32)EBoar::AS_Boar_JumpLoop;
			...
			// jump end task node
			{
				...
				tTaskJumpEndDesc.m_spOwnerGameObject = spBoar;
				tTaskJumpEndDesc.m_wstrBTNodeName = L"JumpEndTask";
				tTaskJumpEndDesc.m_eBTNodeType = EBTNodeType::TASK;

				// move task desc
				tTaskJumpEndDesc.m_iAnimationID = (int32)EBoar::AS_Boar_JumpEnd;
		...
		// damage task node
		{
			...
			tTaskDamageDesc.m_spOwnerGameObject = spBoar;
			tTaskDamageDesc.m_wstrBTNodeName = L"DamageTask";
			tTaskDamageDesc.m_eBTNodeType = EBTNodeType::TASK;

			// damage task desc
			tTaskDamageDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Damage;
		...
		// attack sequence node
		{
			...
			// look at task node
			{
				...
				tTaskLookAtDesc.m_spOwnerGameObject = spBoar;
				tTaskLookAtDesc.m_wstrBTNodeName = L"AttackPreLookAtTask";
				...
				tTaskLookAtDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Walk;
			...
			// encount task node
			{
				...
				tTaskEncountDesc.m_spOwnerGameObject = spBoar;
				...
				tTaskEncountDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Encount;
			...
			// chase task node
			{
				...
				tTaskChaseDesc.m_spOwnerGameObject = spBoar;
				tTaskChaseDesc.m_wstrBTNodeName = L"AttackChaseTask";
				tTaskChaseDesc.m_eBTNodeType = EBTNodeType::TASK;

				// chase task desc;
				tTaskChaseDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Run;
			...
			// look at task node
			{
				...
				FTaskLookAtDesc tTaskLookAtDesc;
				tTaskLookAtDesc.m_spOwnerGameObject = spBoar;
				tTaskLookAtDesc.m_wstrBTNodeName = L"AttackPostLookAtTask";
				tTaskLookAtDesc.m_eBTNodeType = EBTNodeType::TASK;
			...
			{
				// task node
				FTaskAttackStartDesc tAttackStartDesc;
				tAttackStartDesc.m_spOwnerGameObject = shared_from_this(); 
				tAttackStartDesc.m_eBTNodeType = EBTNodeType::TASK;
				tAttackStartDesc.m_wstrBTNodeName = L"AttackStartTask";

				// attack start node
				tAttackStartDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Tossin_Start;
				spAttackSequenceNode->AddBTNode(CTaskAttackStart::Create(make_shared<FTaskAttackStartDesc>(tAttackStartDesc)));
			}

			// boar attack loop node
			{
				// node desc
				FTaskBoarAttackLoopDesc tAttackLoopDesc;
				...
				tAttackLoopDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Tossin_Loop;
				spAttackSequenceNode->AddBTNode(CTaskBoarAttackLoop::Create(make_shared<FTaskBoarAttackLoopDesc>(tAttackLoopDesc)));
			}

			// attack end node
			{
				// task node
				FTaskAttackEndDesc tAttackEndDesc;
				tAttackEndDesc.m_spOwnerGameObject = shared_from_this();
				tAttackEndDesc.m_eBTNodeType = EBTNodeType::TASK;
				tAttackEndDesc.m_wstrBTNodeName = L"AttackEndTask";

				// attack start node
				tAttackEndDesc.m_iAttackEndAnimationID = (int32)EBoar::AS_Boar_Tossin_End;
				spAttackSequenceNode->AddBTNode(CTaskAttackEnd::Create(make_shared<FTaskAttackEndDesc>(tAttackEndDesc)));
			}
		}
		...
		// runaway node
		{
			...
			tRunawayDesc.m_spOwnerGameObject = spBoar;
			...
			tRunawayDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Run;
		...
		// sleep sequence node
		{
			...
			// sleep deco node
			{
				...
				tDecoSleepDesc.m_spOwnerGameObject = spBoar;
			...
			// sleep start task node
			{
				...
				tTaskSleepStartDesc.m_spOwnerGameObject = spBoar;
				...
				tTaskSleepStartDesc.m_iAnimationID = (int32)EBoar::AS_Boar_SleepStart;
			...
			// sleep end task node
			{
				...
				tTaskSleepEndDesc.m_spOwnerGameObject = spBoar;
				tTaskSleepEndDesc.m_wstrBTNodeName = L"SleepEndTask";
				tTaskSleepEndDesc.m_eBTNodeType = EBTNodeType::TASK;

				// sleep end task desc
				tTaskSleepEndDesc.m_iAnimationID = (int32)EBoar::AS_Boar_SleepEnd;
		...
		// wander sequence node
		{
			...
			// idle task node
			{
				...
				tTaskIdleDesc.m_spOwnerGameObject = spBoar;
				tTaskIdleDesc.m_wstrBTNodeName = L"IdleTask";
				tTaskIdleDesc.m_eBTNodeType = EBTNodeType::TASK;

				// idle task desc
				tTaskIdleDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Idle;
			...
			// move task node
			{
				...
				tTaskMoveDesc.m_spOwnerGameObject = spBoar;
				tTaskMoveDesc.m_wstrBTNodeName = L"WanderMoveTask";
				tTaskMoveDesc.m_eBTNodeType = EBTNodeType::TASK;

				// move task desc
				tTaskMoveDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Walk;
				tTaskMoveDesc.m_vSpawnPosition = _spBoarDesc->m_vInitPosition;
			...
			// rest task node
			{
				...
				// rest task desc
				tTaskRestDesc.m_iAnimationID = (int32)EBoar::AS_PinkCat_Rest02;
...
void Client::CBoar::InitializePartnerPalBT(const std::shared_ptr<FBoarDesc> _spBoarDesc, bool _bInit)
{
	shared_ptr<CGameObject> spBoar = shared_from_this();
	...
	// behavior tree node
	{
		...
		// attack sequence node
		{
			...
			// withdraw task node
			{
				...
				tTaskWithdrawDesc.m_spOwnerGameObject = spBoar;
				tTaskWithdrawDesc.m_wstrBTNodeName = L"WithdrawTask";
				tTaskWithdrawDesc.m_eBTNodeType = EBTNodeType::TASK;

				// withdraw task desc
				tTaskWithdrawDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Idle;
		...
		// follow sequence node
		{
			...
			// follow task node
			{
				...
				tTaskFollowDesc.m_spOwnerGameObject = spBoar;
				...
				tTaskFollowDesc.m_iWalkAnimationID = (int32)EBoar::AS_Boar_Walk;
				tTaskFollowDesc.m_iRunAnimationID = (int32)EBoar::AS_Boar_Run;
			...
			// turn task node
			{
				...
				tTaskTurnDesc.m_spOwnerGameObject = spBoar;
				...
				tTaskTurnDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Idle;
...
void Client::CBoar::InitializeSlavePalBT(const std::shared_ptr<FBoarDesc> _spBoarDesc, bool _bInit)
{
	shared_ptr<CGameObject> spBoar = shared_from_this();
	...
	// behavior tree node
	{
		...
		// interacted sequence node
		{
			...
			// interacted deco node
			{
				...
				tDecoInteractedDesc.m_spOwnerGameObject = spBoar;
			...
			// react task node
			{
				...
				tTaskReactDesc.m_spOwnerGameObject = spBoar;

				// react task desc
				tTaskReactDesc.m_iEatAnimationID = (int32)EBoar::AS_Boar_Eat;
				tTaskReactDesc.m_iPetAnimationID = (int32)EBoar::AS_Boar_Petting;
				tTaskReactDesc.m_iSlaughterAnimationID = (int32)EBoar::AS_Boar_Carrying;
			...
			// emote task node
			{
				...
				tTaskEmoteDesc.m_spOwnerGameObject = spBoar;

				// react task desc
				tTaskEmoteDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Idle;
		...
		/*
		{
			...
			// hunger deco node
			{
				...
				tDecoHungerDesc.m_spOwnerGameObject = spBoar;
			...
			// notice task node
			{
				...
				tTaskNoticeDesc.m_spOwnerGameObject = spBoar;

				// notice task desc
				tTaskNoticeDesc.m_iAnimationID = (int32)EBoar::AS_PinkCat_Idle;
			...
			// move task node
			{
				...
				// move task desc
				tTaskMoveDesc.m_iAnimationID = (int32)EBoar::AS_PinkCat_Run;
			...
			// react task node
			{
				...
				// move task desc
				tTaskReactDesc.m_iEatAnimationID = (int32)EBoar::AS_PinkCat_Eat;
		...
		*/
		...
		// todo sequence node
		{
			...
			// notice task node
			{
				...
				// notice task desc
				tTaskNoticeDesc.m_iAnimationID = (int32)EBoar::AS_Boar_Idle;
				tTaskNoticeDesc.m_eMonsterNoticeType = EMonsterNoticeType::LIGHT_BULB;
			...
			// todo task node
			{
				...
				tTaskToDoDesc.m_spOwnerGameObject = spBoar;

				// todo task desc
				tTaskToDoDesc.m_iMineAnimationID = (int32)EBoar::AS_Boar_Mining;
		...
		// wander sequence node
		{
			...
			// rest task node
			{
				// node desc
				FTaskRestDesc tTaskRestDesc;
				tTaskRestDesc.m_spOwnerGameObject = spBoar;
				tTaskRestDesc.m_wstrBTNodeName = L"RestTask";
				tTaskRestDesc.m_eBTNodeType = EBTNodeType::TASK;

				// rest task desc
				tTaskRestDesc.m_iAnimationID = (int32)EBoar::AS_PinkCat_Rest03;
...
HRESULT Client::CBoar::ReInitializePalBT(EMonsterAIType _eMonsterAIType)
{
...
void Client::CBoar::LookAt(const Vector3& _vPosition)
{
...
void Client::CBoar::Turn(const Vector3& _vAxis, float _fAngularSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::MoveForward(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::MoveBackward(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::MoveRight(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::MoveLeft(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::MoveUp(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::MoveDown(float _fSpeed)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::SetPosition(const Vector3& _vPosition)
{
	if (!m_spBodySphereComponent) { return; }
...
const std::optional<Vector3> Client::CBoar::GetPosition() const
{
...
void Client::CBoar::ChangeMouth(EPalMouthType _ePalMouthType)
{
	return;
}

void Client::CBoar::UpdateBlackboardValues()
{
	CMonster::UpdateBlackboardValues();
}

void Client::CBoar::OnTriggerEnter(CComponent* _pSrcComponent, CComponent* _pDstComponent)
{
	...
	if (_pDstComponent->GetComponentType() == EComponentType::CONTROLLER)
	{
		...
		// 야생 돌진 돼지가 플레이어를 공격합니다.
		if (m_eMonsterAIType == EMonsterAIType::WILD_PAL)
		{
			if (
				GetBoolValue(EBlackboardBoolValue::BOOL_ATTACK) == true		// 야생 돌진 돼지가 공격하는 중이고
				&& (eSrcCollisionLayer == ECollisionLayer::MONSTER_WEAPON)	// 야생 돌진 돼지의 공격 콜라이더가
				...
				)
			{
				...
				spPlayer->AddForce(vForceDirection, CClientLoop::s_spClientDataSheet->k_mapKnockBackValue[EMonsterType::BOAR]);
	...
	// 파트너 돌진 돼지가 야생 팰을 공격합니다.
	if (m_eMonsterAIType == EMonsterAIType::PARTNER_PAL)
	{
		if
		(
			GetBoolValue(EBlackboardBoolValue::BOOL_ATTACK) == true	// 파트너 돌진 돼지가 공격하는 중이고
			&& (eSrcCollisionLayer == ECollisionLayer::ALLY_WEAPON)	// 파트너 돌진 돼지의 공격 콜라이더가
...
void Client::CBoar::OnCollisionEnter(CComponent* _pSrcComponent, CComponent* _pDstComponent)
{
	CMonster::OnCollisionEnter(_pSrcComponent, _pDstComponent);
}

void Client::CBoar::SetAllyCollisionLayer()
{
...
std::shared_ptr<CColliderComponent> Client::CBoar::GetBodyColliderComponent()
{
	return m_spBodySphereComponent;
}

void Client::CBoar::SetLinearVelocity(const PxVec3& _vLinearVelocity)
{
	if (!m_spBodySphereComponent) { return; }
...
void Client::CBoar::SetAngularVelocity(const PxVec3& _vLinearVelocity)
{
	if (!m_spBodySphereComponent) { return; }
