...
void Client::CSheepBall::OnTriggerEnter(CComponent* _pSrcComponent, CComponent* _pDstComponent)
{
	...
	if (_pDstComponent->GetComponentType() == EComponentType::CONTROLLER)
	{
		...
		if (m_eMonsterAIType == EMonsterAIType::WILD_PAL)
		{
			...
			if
				(
					GetBoolValue(EBlackboardBoolValue::BOOL_ATTACK) == true		// 야생 도로롱이 공격하는 중이고
					&& (eSrcCollisionLayer == ECollisionLayer::MONSTER_WEAPON)	// 야생 도로롱의 공격 콜라이더가
	...
	// 파트너 도로롱이 야생 팰을 공격합니다.
	if (m_eMonsterAIType == EMonsterAIType::PARTNER_PAL)
	{
		if
			(
				GetBoolValue(EBlackboardBoolValue::BOOL_ATTACK) == true	// 파트너 도로롱이 구르는 중이고
				&& (eSrcCollisionLayer == ECollisionLayer::ALLY_WEAPON)	// 파트너 도로롱의 공격 콜라이더가
				&& (eDstCollisionLayer == ECollisionLayer::MONSTER)		// 야생 팰과 부딪친 경우
			)
		{
			ModifyBoolValue(EBlackboardBoolValue::BOOL_ATTACKED_TARGET, true);
