...
std::shared_ptr<CTaskDreamDemonAttackLoop> Client::CTaskDreamDemonAttackLoop::Create(const std::shared_ptr<FTaskDreamDemonAttackLoopDesc>& _spTaskAttackLoopDesc)
{
	shared_ptr<CTaskDreamDemonAttackLoop> spTaskNode = make_shared<CTaskDreamDemonAttackLoop>();
	ENSUREF(spTaskNode->Initialize(_spTaskAttackLoopDesc), L"Failed to initialize dreamdemon attack loop task node.");
	return spTaskNode;
}

HRESULT Client::CTaskDreamDemonAttackLoop::Initialize(const shared_ptr<FBTNodeDesc>& _spNodeDesc)
{
	// task desc
	...
	shared_ptr<FTaskDreamDemonAttackLoopDesc> spTaskAttackLoopDesc = static_pointer_cast<FTaskDreamDemonAttackLoopDesc>(_spNodeDesc);
...
void Client::CTaskDreamDemonAttackLoop::OnNodeInit(float _fDeltaSecond)
{
...
void Client::CTaskDreamDemonAttackLoop::OnNodeEnter(float _fDeltaSecond)
{
	// 표정을 변경합니다.
	...
	ActivateProjectileEffectGroup(0, m_wpTarget.lock(), m_spMonster->GetPosition().value() + m_spMonster->GetForwardVector().value() + Vector3(0.f, 2.f, 0.f));
}

EBTNodeResult Client::CTaskDreamDemonAttackLoop::ExecuteTaskNode(float _fDeltaSecond)
{
	// 공격할 대상이 없거나 죽은 경우
...
void Client::CTaskDreamDemonAttackLoop::Release()
{
	m_spMonster = nullptr;
	m_wpTarget.reset();
...
void Client::CTaskDreamDemonAttackLoop::InitializeProjectileEffectGroups(const std::wstring& _wstrProjectileEffectGroupName, ECollisionLayer _eCollisionLayer, ECollisionFlag _eCollisionFlag)
{
	...
	tProjectileEffectGroupDesc.m_eOwnerMonsterType = EMonsterType::DREAM_DEMON;
...
HRESULT Client::CTaskDreamDemonAttackLoop::ActivateProjectileEffectGroup(int32 _iProjectileSlot, std::shared_ptr<CCreature> _spTarget, const Vector3 _vActivatedPosition)
{
